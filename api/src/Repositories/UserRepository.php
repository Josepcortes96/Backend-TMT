<?php
    ...
    class UserRepository implements UserRepositoryInterface{
            ...
            public function relacionCentroUsuario(int $userId, int $centroId, string $rol): void {
                ...
                $col = str_replace('centro_', '', $tabla);
                $stmt = $this->pdo->prepare("INSERT INTO $tabla (id_centro, id_$col) VALUES (:centro_id, (SELECT id_$col FROM $col" . "s WHERE id_usuario = :id_usuario))");
                $stmt->execute([':centro_id' => $centroId, ':id_usuario' => $userId]);
            }
            ...
            public function eliminarRelacionDietaRol(int $id, string $rol): void {
                $tabla = match ($rol) {
                    'Cliente' => 'dieta_cliente',
                    'Preparador' => 'dieta_preparador',
                    'Propietario' => 'dieta_propietario',
                    default => throw new Exception("Rol no válido")
                };
                $col = str_replace('dieta_', '', $tabla);
                $stmt = $this->pdo->prepare("DELETE FROM $tabla WHERE id_$col = :id");
                $stmt->execute([':id' => $id]);
            }

            public function capturarCentroId(int $id, string $rol): ?int {
                $rolCol = strtolower($rol);
                $stmt = $this->pdo->prepare("SELECT id_centro FROM centro_$rolCol WHERE id_$rolCol = (SELECT id_$rolCol FROM {$rolCol}s WHERE id_usuario = :id)");
                $stmt->execute([':id' => $id]);
            ...
            public function capturarDietaId(int $id, string $rol): ?int {
                $rolCol = strtolower($rol);
                $stmt = $this->pdo->prepare("SELECT id_dieta FROM dieta_$rolCol WHERE id_$rolCol = (SELECT id_$rolCol FROM {$rolCol}s WHERE id_usuario = :id)");
                $stmt->execute([':id' => $id]);
                return $stmt->fetchColumn() ?: null;
            }
            ...
            public function relacionDietaUsuario(int $userId, ?int $dietaId, string $rol): void {
                ...
                $col = str_replace('dieta_', '', $tabla);
                $stmt = $this->pdo->prepare("INSERT INTO $tabla (id_dieta, id_$col) VALUES (:dieta_id, (SELECT id_$col FROM $col" . "s WHERE id_usuario = :id_usuario))");
